...
_R103_TRAIN_RUN_SEEDS = {f"alfworld_train_full_120_r103_seed{seed}": seed for seed in (42, 43, 44)}
...
def _train_protocol(config: dict[str, Any]) -> tuple[str, int, int, int]:
    ...
    if name in _R103_TRAIN_RUN_SEEDS:
        return "r103_full120", _R103_TRAIN_RUN_SEEDS[name], 20, 120
...
def _r7_train_reference_manifest(
    ...
    if protocol not in {
        "r7_full120", "r8_full120", "r9_full120", "r92_full120", "r10_full120", "r1021_full120", "r103_full120",
    }:
        return None
...
def _selection(config: dict[str, Any]) -> tuple[list[str], int, int]:
    ...
    if per_type != expected_per_type or total != expected_total:
        label = (
            "R10.3 Full-120" if protocol == "r103_full120" else
...
def _validate_formal_config(config: dict[str, Any], output_dir: Path) -> None:
    ...
    if protocol in {"r9_full120", "r92_full120", "r10_full120", "r1021_full120", "r103_full120"}:
        required_lifecycle = {
    ...
    if protocol == "r92_full120" and config.get("repair_revision") != "R9.2":
        mismatches.append("R9.2 formal train requires repair_revision='R9.2'")
...
def run(config_path: str | Path, *, resume: bool = False, provider_recovery: bool = False) -> int:
    ...
    if protocol in {'r1021_full120', 'r103_full120'}:
    ...
    with AtomicSkillGraphSystem(config, readonly=False) as system:
        ...
        report_stem = {
            ...
            "r103_full120": f"train_full_120_r103_seed{experiment_seed}",
        ...
        report_title = {
            ...
            "r103_full120": f"AtomicSkillGraph v3 ALFWorld Full-120 R10.3 Train (seed {experiment_seed})",
            "r92_full120": (
        ...
        freeze_provenance = {
            **(r103_metadata_for(manifest.metadata) if manifest.metadata.get("repair_revision") == "R10.3" else {}),
