...
if [[ -z "$OUTPUT" || ! "$MODE" =~ ^(smoke|formal|resume)$ ]]; then
  echo "Usage: bash experiments/baselines/launch_static_skill.sh smoke|formal|resume OUTPUT [SMOKE_QUALIFICATION]" >&2
  exit 2
fi
...
exec "$B1_PY" -m experiments.baselines.b1_static_skill.campaign "${ARGS[@]}"
