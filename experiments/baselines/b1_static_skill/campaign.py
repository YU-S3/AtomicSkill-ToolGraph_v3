"""B1 smoke / formal Test134 × three seeds, with verified task-boundary resume."""
import argparse
...
from .driver import METHOD, StaticSkillDriver, file_hash, usage_totals, initial_authority, frozen_skill
...
def validate_config(cfg):
    ...
    if any(cfg.get(k) != v for k,v in fixed.items()):
        raise ValueError("Frozen B1 identity/actions/seeds differ")
    if cfg["model"].get("provider_completion_cap") != 65536:
        raise ValueError("B1 reasoning-aware completion allowance must be 65536")
    if cfg["parallel"] != dict(seed_lanes=3, allowed_global_caps=[48,36,24], mp_start_method="spawn"):
        raise ValueError("B1 frozen concurrency policy differs")
    if cfg["provider_transport"] != dict(sdk_max_retries=0, application_retry_limit=5,
        ...
        raise ValueError("B1 transport retry policy differs")
    if any(k in cfg for k in ("train", "validation", "skill_text", "initial_skill", "bank")):
        raise ValueError("B1 cannot configure training or override the pinned initial skill")
...
def source_identity(source):
    ...
    # Only the pinned initial skill is exposed to the agent, never training data.
    upstream = verify_runtime_tree(source, "skillopt", lock)
    runtime = runtime_python_receipt()
    if not runtime["venv_active"]:
        raise ValueError("B1 must use the configured virtual environment interpreter")
    if git("status", "--porcelain", "--", "src", "experiments", "configs"):
        raise ValueError("Commit source changes before B1 qualification/formal")
    return dict(commit=git("rev-parse", "HEAD"), code_hash=hash_code(REPO),
        upstream=upstream, upstream_commit=lock["skillopt"]["commit"],
        source=str(source), python=runtime, initial_skill=initial_authority(source),
        packages={p:importlib.metadata.version(p) for p in ("alfworld","textworld","openai")})


def smoke_tasks():
    # Engineering probes only, never part of B1's formal Train/Val costs or state.
...
def verify_receipt(path, identity, cfg):
    ...
    if receipt.get("passed") is not True or receipt.get("identity") != identity or receipt.get("config_hash") != sha256_json(cfg):
        raise ValueError("B1 smoke qualification missing/failed or source/config/runtime changed")
    if receipt.get("real_episodes") != 6 or receipt.get("seeds") != [42,43,44] or receipt.get("checks_passed") is not True:
        raise ValueError("B1 smoke lacks complete three-seed six-family checks")
    root = path.parent
    for name, digest in receipt["evidence_hashes"].items():
        target = (root/name).resolve()
        target.relative_to(root.resolve())
        if file_hash(target) != digest:
            raise ValueError("B1 smoke evidence changed")
    return receipt


def preflight(root, source, cfg, campaign_id):
    from experiments.baselines.b0_dynamic.load_probe import loaded_workers
        ...
        try:
            with loaded_workers(str(source), str(Path(os.environ["ALFWORLD_DATA"])/task.gamefile_rel),
                ...
                if not report["passed"]:
                    raise RuntimeError("B1 provider load failed; not a memory fallback")
            ...
            continue
    raise RuntimeError("B1 cannot pass even global24 real memory load")
...
def load_record(path, task, seed, digest, phase):
    ...
    if record.method != METHOD or record.evolution_llm_calls != 0:
        raise ValueError("B1 record method/no-learning identity differs")
    if (record.task_id, record.manifest_index, record.run_seed, record.phase, record.gamefile_hash,
        ...
        raise ValueError("Completed B1 task identity/freeze mismatch")
    return record
...
def lane_run(spec, seed, tasks, stop=None):
    ...
    frozen = driver.freeze(lane, spec["source"])
    write_json(lane/"run_manifest.json", dict(method=METHOD, seed=seed, identity=spec["identity"],
        ...
        initial_skill=spec["identity"]["initial_skill"]))
    pending = []
    for task in tasks:
        ...
        if (path/"completion.json").exists():
            ...
            pending.append(dict(episode_dir=str(path), task=task.to_dict(), frozen=str(lane/"frozen"),
                config=spec["config"], source=spec["source"], initial_skill=spec["identity"]["initial_skill"],
                seed=seed, phase=spec["phase"],
        ...
        while active:
            ...
            for future in done:
                ...
                if not result["passed"]:
                    ...
                    if stop is not None:
                        stop.set()
                print(f"B1 seed={seed} task={job['task']['task_id']} passed={result['passed']}", flush=True)
            if not failures:
                for _ in done:
                    dispatch()
    if failures:
...
def smoke_checks(root):
    ...
    authority = read_json(root/"campaign_lock.json")["identity"]["initial_skill"]
    skills = [frozen_skill(FrozenArtifact.load(root/f"seed_{seed}"/"frozen"), authority)
              for seed in (42,43,44)]
    expected_prefix = _build_skill_prompt(skills[0]) + "\n"
    ...
    lanes = [read_json(root/f"seed_{seed}"/"summary.json") for seed in (42,43,44)]
    checks = dict(six_episodes=len(records)==6,
        ...
        prompt_evidence_complete=bool(events) and
            {r["logical_call_id"] for r in responses}=={e["call_id"] for e in events},
        exact_skill_injection=bool(responses) and all(
            len([m for m in r["messages"] if m["role"]=="user"])==1 and
            next(m["content"] for m in r["messages"] if m["role"]=="user").startswith(expected_prefix)
            for r in responses),
        identical_frozen_seeds=len(set(skills))==1,
        no_training=all(r["training_cost"]==StaticSkillDriver().train() for r in lanes),
        no_learning=all(r["method_metrics"]["initial_skill_sha256"]==authority["initial_skill_sha256"]
                       and r["evolution_llm_calls"]==0 for r in records),
        ...
        usage_complete=bool(physical) and all(isinstance(p.get(k),int) and p[k]>=0
            for p in physical for k in ("prompt_tokens","completion_tokens","reasoning_tokens","visible_completion_tokens")))
...
def run(args):
    ...
    lease = nullcontext() if is_smoke else _method_campaign_lease(lease_root(), owner="b1_"+root.name)
    with lease:
        if args.mode == "resume":
            ...
            if spec["identity"]!=identity or spec["config"]!=cfg or spec["test_digest"]!=test_digest or spec["alfworld_data"]!=data:
                raise ValueError("B1 resume source/config/runtime/data identity changed")
            ...
            if spec["phase"]!="test" or not load["passed"] or spec["cap"]!=load["selected_cap"] or spec["cap"] not in (48,36,24):
                raise ValueError("B1 resume must retain qualified formal concurrency")
            for label in ("memory", "provider"):
                if file_hash(load[label+"_report"]) != load[label+"_sha256"]:
                    raise ValueError("B1 load qualification changed")
        else:
            if not is_smoke:
                if not args.qualification:
                    raise ValueError("B1 formal requires a passing --qualification")
        ...
        if source_identity(source)!=identity:
            raise ValueError("Source/runtime changed during B1")
        ...
        if passed and is_smoke:
            ...
            write_json(root/"smoke_qualification.json", dict(passed=True, identity=identity,
                ...
                real_episodes=6, initial_skill=identity["initial_skill"], checks_passed=True, evidence_hashes=evidence))
        elif passed:
            from experiments.baselines.report_campaign import build_campaign_report
            paper = build_campaign_report({METHOD:[root/f"seed_{s}" for s in (42,43,44)]})
            write_json(root/"paper_report.json", paper)
            lines = ["# B1 Static SkillOpt Seed — Test134 × 3 seeds", "", "Status: complete", "",
                     "Pinned initial.md only; no training, validation selection or persistent experience.", "",
        ...
        if not is_smoke:
            write_json(root/f"campaign_summary_{time.time_ns()}.json", summary)
            write_json(root/"campaign_summary.json", summary)
        print(f"B1 {args.mode}: passed={passed}; output={root}", flush=True)
...
def main(argv=None):
    ...
    parser.add_argument("--config", default=str(REPO/"configs/baselines/b1_static_skill.yaml"))
    ...
    try:
        ...
        print(f"B1 stopped: {type(exc).__name__}: {sanitize_error_text(exc)}", file=sys.stderr)
        return 1
