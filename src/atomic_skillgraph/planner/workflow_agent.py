...
WORKFLOW_SCHEMA: dict[str, Any] = {
    "type": "object", "required": ["steps", "control_sequence", "data_edges", "dependency_edges", "requirement_coverage"],
    "properties": {
        "steps": {"type": "array", "items": {"type": "object"}},
        "control_sequence": {"type": "array", "items": {"type": "string"}},
        "data_edges": {"type": "array", "items": {"type": "object"}},
        "dependency_edges": {"type": "array", "items": {"type": "object"}},
        "requirement_coverage": {"type": "object"},
    },
}


def _json(turn: Any) -> dict[str, Any]:
    try:
        return json.loads(str(getattr(turn, "content", "") or ""))
    except json.JSONDecodeError as exc:
        raise ValueError("Workflow structured output was not valid JSON") from exc
...
class WorkflowAgent:
    ...
    def propose(self, task: Any, contract: Any, requirements: Any, candidates: Any, existing_edges: Any, hints: Any) -> PlannerWorkflowProposal:
        ...
        prompt = (
            ...
            "origin=planner_proposed and every reused edge origin=existing_active with its real id. JSON only.\n"
        ...
        return _proposal(_json(self.session.next_turn(prompt, structured_output_schema=WORKFLOW_SCHEMA)))
    ...
    def repair(self, proposal: PlannerWorkflowProposal, validation: Any, authoritative_contracts: Any, existing_edges: Any) -> PlannerWorkflowProposal:
        ...
        prompt = (
            ...
            "forge existing edges, or add another control path. JSON only.\n"
