...
def _contract_covered(
    ...
    for target in contract.target_effects:
        compatible = [
            ...
            if effect.predicate.casefold() == target.predicate.casefold()
            and set(target.args).issubset(arguments)
        ]
    ...
    for rule in contract.cardinality_constraints:
        ...
        matching = [item for item in offered if item[0].predicate.casefold() == predicate.casefold()]
    ...
    for constraint in contract.identity_constraints:
        if constraint.relation is IdentityRelation.SAME_AS:
            if constraint.left_role == constraint.right_role:
                ...
                left = {item[2][constraint.left_role] for item in offered if constraint.left_role in item[2]}
                right = {item[2][constraint.right_role] for item in offered if constraint.right_role in item[2]}
...
class CompositeBuilder:
    def validate_and_build(
        ...
        *,
        ...
        if len(proposal.control_sequence) != len(by_id) or set(proposal.control_sequence) != set(by_id):
            raise ValueError("E2 control sequence must use every canonical occurrence exactly once")
        if not _contract_covered(contract, canonical):
