...
OUTPUT_SEMANTIC_CONSTRAINT_RULES = """output_semantic_constraints is a sparse mapping and may be {}. Each key is a declared output role. compatible_with_input names an exact declared formal input role, not prose, a sample, a relation or lineage. required_resolution is the contract's declared minimum, not a guess from an example value. Under this protocol, an output with compatible_with_input must be declared concrete or relation_verified and the referenced input must satisfy the existing anchor requirements: required and type-compatible, with equal resolution for same-name roles. For a semantic identity return, retain its honest level and omit this extra constraint when it is not applicable. Do not upgrade a declaration merely to pass validation. Fresh concrete outputs require actual effect evidence; pass-through identity must preserve its source identity. Code will not delete invalid constraints or create data-flow edges."""
