...
def test_formal_train_max_task_attempts_config_is_strict_positive_integer() -> None:
    ...
    config_path = repo_root / "configs" / "alfworld_train_full_30_r6.yaml"
    output_dir = repo_root / "runs" / "alfworld_train_full_30_r6"
...
def test_formal_frozen_max_task_attempts_config_is_strict_positive_integer() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    config_path = repo_root / "configs" / "alfworld_frozen_eval_60_r6.yaml"
    output_dir = repo_root / "runs" / "alfworld_frozen_eval_60_r6"
    config = load_config(config_path)
    assert config["experiment"]["max_task_attempts"] == 3
